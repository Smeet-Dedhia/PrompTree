            ...
            return (
              <div key={topic.id} className="relative">
                <div
                  ...
                  className={cn(
                    'group relative flex items-center justify-between px-3 py-2.5 rounded-xl transition-all duration-200 cursor-pointer border border-transparent',
                    isSelected 
                ...
                >
                  ...
                  <div className="flex items-center flex-shrink-0">
                    {/* Count badge which fades out on hover */}
                    <span className={cn(
                      'text-[10px] font-semibold px-2 py-0.5 rounded-full transition-opacity duration-150 group-hover:opacity-0',
                      isSelected 
                    ...
                    {/* Absolute-positioned floating action buttons that overlay in place of count badge on hover */}
                    <div className={cn(
                      "absolute right-2.5 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 flex items-center gap-0.5 transition-all duration-150 z-10 pl-2 rounded-lg",
                      isSelected ? "bg-indigo-50/90" : "bg-slate-50"
                    )}>
                      <button
                        onClick={(e) => handleStartEdit(e, topic.id, topic.name)}
                        className="p-1 rounded-md text-slate-400 hover:bg-slate-105 hover:text-indigo-600 transition-colors"
                        title="Rename Topic"
                      >
                      ...
                      <button
                        ...
                        className="p-1 rounded-md text-slate-400 hover:bg-slate-105 hover:text-rose-600 transition-colors"
                        title="Delete Topic"
                      >
                ...
                {/* Edit Topic Name Popup (floats absolutely below the topic card) */}
                {isEditing && (
                  <div 
                    onClick={(e) => e.stopPropagation()} 
                    className="absolute top-11 left-1.5 right-1.5 bg-white border border-slate-200 rounded-2xl shadow-xl p-3 z-50 animate-in fade-in slide-in-from-top-2 duration-155"
                  >
                    <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Rename Topic</h4>
                    <div className="flex gap-1.5 items-center">
                      <input
                        ...
                        className="flex-1 min-w-0 bg-slate-50 border border-slate-205 rounded-xl px-2.5 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-indigo-105 focus:border-indigo-500 focus:bg-white transition-all duration-150 font-normal"
                        autoFocus
                      />
                      <div className="flex items-center gap-0.5 flex-shrink-0">
                        <button
                          onClick={(e) => handleSaveEdit(e, topic.id)}
                          className="p-1.5 rounded-lg text-emerald-600 hover:bg-emerald-50 hover:text-emerald-700 transition-colors border border-emerald-100"
                          title="Save Changes"
                        >
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-lg text-slate-450 hover:bg-slate-100 hover:text-slate-600 transition-colors border border-slate-200/60"
                          title="Cancel"
                        >
